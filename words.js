...
function randomWord () {
    if (words.length === arrWords.length) arrWords = [];
    let word = words[Math.floor(Math.random() * words.length)];
    arrWords.indexOf(word) === -1 ? arrWords.push(word) : randomWord();
    return word;
}
...
document.addEventListener('DOMContentLoaded', loadedText);
button.addEventListener('click', loadedText);
